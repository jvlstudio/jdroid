...
package com.jdroid.android.leftnavbar;
...
public class LeftNavView extends LinearLayout {
	...
	/*
	 * @Override protected void onDescendantFocusChanged(boolean hasFocus) { super.onDescendantFocusChanged(hasFocus);
	 * if (has(mDisplayOptions, ActionBar.DISPLAY_AUTO_EXPAND)) { setExpanded(hasFocus); } }
	 */
	...
	public int setDisplayOptions(int options) {
		...
		if (has(changes, ActionBar.DISPLAY_USE_LOGO) || has(changes, LeftNavBar.DISPLAY_USE_LOGO_WHEN_EXPANDED)) {
			setHomeMode();
		}
		...
		if (has(changes, LeftNavBar.DISPLAY_AUTO_EXPAND) || has(changes, LeftNavBar.DISPLAY_ALWAYS_EXPANDED)) {
			setExpandedState();
		}
		return changes;
	}
	...
	private void setExpanded(final boolean expanded, boolean animated) {
		...
		if (animated) {
			...
			mWidthAnimator = ValueAnimator.ofInt(getLayoutParams().width, // Starting value.
	...
	private void setContentExpanded(boolean expanded) {
		...
		if (hasCustomView()) {
			getCustomView().setVisibility(expanded ? View.VISIBLE : View.GONE);
	...
	/**
	 * Wrapper around custom views to allow them to use the layout parameters defined in the ActionBar class and still
	 * be displayed within this view group.
	 */
	private static final class CustomViewWrapper extends ViewGroup {
		...
		/**
		 * Locates the top bound of the space available for custom views, expressed in the coordinate system of
		 * {@code parent}.
		 */
		...
		/**
		 * Locates the bottom bound of the space available for custom views, expressed in the coordinate system of
		 * {@code parent}.
		 */
		...
		/**
		 * Should be called after the rest of the left nav has been measured, so that custom views can be sized
		 * according to the space left and the desired gravity.
		 */
		void onPostMeasure(LeftNavView parent) {
			...
			int widthMode = params.width != LayoutParams.WRAP_CONTENT ? MeasureSpec.EXACTLY : MeasureSpec.AT_MOST;
			...
			int heightMode = params.height != LayoutParams.WRAP_CONTENT ? MeasureSpec.EXACTLY : MeasureSpec.AT_MOST;
			...
			if ((vGravity == Gravity.CENTER_VERTICAL) && (params.height == ViewGroup.LayoutParams.MATCH_PARENT)
					&& (availableInTopHalf > 0) && (availableInBottomHalf > 0)) {
				// Attempt to center if there's enough space to center.
				heightValue = Math.min(availableInTopHalf, availableInBottomHalf) * 2;
			}
			
			mView.measure(MeasureSpec.makeMeasureSpec(widthValue, widthMode),
		...
		void onPostLayout(LeftNavView parent) {
			...
			// available space.
			/*
			 * int superContainerHeight = parent.mBottom - parent.mTop - parent.mPaddingTop - parent.mPaddingBottom;
			 */
			int superContainerHeight = parent.getBottom() - parent.getTop() - parent.getPaddingTop()
