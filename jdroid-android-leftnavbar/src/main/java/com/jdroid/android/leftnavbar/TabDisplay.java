...
class TabDisplay {
	...
	private void onSelectionChanged(TabImpl oldSelection, TabImpl newSelection) {
		...
		if (mContext instanceof Activity) {
			transaction = ((Activity)mContext).getFragmentManager().beginTransaction().disallowAddToBackStack();
		}
	...
	private final class TabAdapter extends ArrayAdapter<TabImpl> {
		...
		/**
		 * {@code true} if selection changes should trigger a callback and any visual change; otherwise the selection is
		 * just remembered.
		 */
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (!mCachedViews.containsKey(tab)) {
				TabFrame frame = (TabFrame)LayoutInflater.from(getContext()).inflate(R.layout.leftnav_bar_tab, parent,
					false);
