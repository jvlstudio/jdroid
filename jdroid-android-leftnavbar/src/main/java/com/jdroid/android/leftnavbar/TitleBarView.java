...
public class TitleBarView extends RelativeLayout {
		...
		switch (value) {
			...
			default:
				if ((Window.PROGRESS_START <= value) && (value <= Window.PROGRESS_END)) {
					...
					mHorizontalProgress.setSecondaryProgress(value - Window.PROGRESS_SECONDARY_START);
				}
				break;
		}
	}
	...
	public int getApparentHeight() {
		return isVisible() ? getContext().getResources().getDimensionPixelSize(R.dimen.title_bar_apparent_height) : 0;
	}
	
	public void setProgressVisible(boolean visible) {
		setCircularProgress(visible ? Window.PROGRESS_VISIBILITY_ON : Window.PROGRESS_VISIBILITY_OFF);
	}
}
