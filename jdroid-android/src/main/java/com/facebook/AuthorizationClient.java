...
class AuthorizationClient implements Serializable {
			...
			public void onBatchCompleted(RequestBatch batch) {
				try {
					...
					if ((fbids.size() == 2) && (fbids.get(0) != null) && (fbids.get(1) != null)
						...
						AccessToken tokenWithPermissions = AccessToken.createFromTokenWithRefreshedPermissions(
							pendingResult.token, tokenPermissions);
	...
	class WebViewAuthHandler extends AuthHandler {
		...
		boolean tryAuthorize(final AuthorizationRequest request) {
			...
			WebDialog.Builder builder = new AuthDialogBuilder(getStartActivityDelegate().getActivityContext(),
					applicationId, parameters).setOnCompleteListener(listener);
		...
		void onWebDialogComplete(AuthorizationRequest request, Bundle values, FacebookException error) {
			...
			if (values != null) {
				...
				AccessToken token = AccessToken.createFromWebBundle(request.getPermissions(), values,
		...
		void getTokenCompleted(AuthorizationRequest request, Bundle result) {
			...
			if (result != null) {
				...
				if ((currentPermissions != null)
					...
					AccessToken token = AccessToken.createFromNativeLogin(result,
	...
	class KatanaLoginDialogAuthHandler extends KatanaAuthHandler {
		...
		boolean onActivityResult(int requestCode, int resultCode, Intent data) {
			if (NativeProtocol.isServiceDisabledResult20121101(data)) {
				...
				if (resultCode == Activity.RESULT_CANCELED) {
					outcome = Result.createCancelResult(data.getStringExtra(NativeProtocol.STATUS_ERROR_DESCRIPTION));
		...
		private Result handleResultOk(Intent data) {
			...
			if (errorType == null) {
				return Result.createTokenResult(AccessToken.createFromNativeLogin(extras,
	...
	static class AuthorizationRequest implements Serializable {
		...
		AuthorizationRequest(SessionLoginBehavior loginBehavior, int requestCode, boolean isLegacy,
			...
			previousAccessToken = validateSameFbidAsToken;
